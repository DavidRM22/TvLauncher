...
public class AppAdapter extends RecyclerView.Adapter<AppAdapter.AppViewHolder> {
    ...
    public void onBindViewHolder(@NonNull AppViewHolder holder, int position) {
        ...
        holder.cardView.setOnClickListener(v -> onAppClickListener.onAppClick(appItem));
        holder.cardView.setOnFocusChangeListener((v, hasFocus) -> {
            float scale = hasFocus ? 1.06f : 1.0f;
            holder.cardView.animate().scaleX(scale).scaleY(scale).setDuration(120).start();
            holder.cardView.setStrokeWidth(hasFocus ? 3 : 0);
            holder.appNameText.setAlpha(hasFocus ? 1.0f : 0.88f);
        });
    }
